...
accelerate launch --debug --num_processes="$((N_TASKS))" --num_machines="${JOB_NUM_NODES}" --mixed_precision bf16 --machine_rank="${NODEID}" --main_process_ip="${MASTER_ADDR}" --main_process_port="${MASTER_PORT}" --dynamo_backend=no src/main.py --jid="$(uuidgen)" --run-id="feb22-rej-n512fixed-1e5" --test-ckpt=$MODEL_PATH --use-cached-dataset --use-gpu --use-logfile --env=".env" --epochs=100 --test-bs=32 --llm="qwen-2.5-1.5B" --room-type="all" --use-wandb --lambda-instr-exp=0.0 --do-rej-sft --dvc-batch-size=4 --rej-num-gen=16 --gas-steps=16 --rej-learning-rate=1e-5 --use-vllm --do-augm --num-eval-workers=16 --rej-samples-per-epoch=512 --rej-max-samples-per-prompt=2 --rej-use-fixed-subset