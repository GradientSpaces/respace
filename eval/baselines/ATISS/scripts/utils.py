...
def get_textured_objects_in_scene(scene, ignore_lamps=False):
    ...
    for furniture in scene.bboxes:
        ...
        try:
            ...
            # custom code martin
            try:
                # Try to load the furniture with normal texture
                raw_mesh = TexturedMesh.from_file(model_path)
            except (IndexError, ValueError, IOError) as e:
                print(f"Failed to load texture for {model_path}: {e}")
                # Create a default mesh without texture
                from simple_3dviz.renderables import Mesh
                import trimesh
                
                # Load the mesh geometry without materials
                mesh = trimesh.load(model_path, force='mesh', process=False)
                
                # Create a simple mesh instead with default color
                raw_mesh = Mesh.from_faces(
                    vertices=mesh.vertices,
                    faces=mesh.faces,
                    colors=np.array([0.8, 0.8, 0.8])  # Gray color
                )
                print(f"Created fallback mesh for {model_path}")
            ...
            print(f"Failed to load {model_path}: {e}.. ignoring mesh entirely !")
...
def scene_from_args(args):
    ...
    far_plane = 2.0 * args.camera_position[1]
...
def export_scene(output_directory, trimesh_meshes, names=None):
    ...
    try:
        ...
        scene_path = os.path.join(output_directory, "00_complete_scene.obj")
        scene.export(scene_path)
    except Exception as e:
        print(f"Warning: Failed to export scene: {e}")
